...
/** `sm`: dùng cho khu vực cần tối ưu chiều cao (vd. thanh bộ lọc) — mặc định `md` giữ nguyên hành vi cũ ở mọi nơi khác. */
...
export function Field({
  ...
  return (
    <label
      className={cn("flex flex-col", uiSize === "sm" ? "gap-1 text-xs" : "gap-1.5 text-sm", className)}
      {...props}
    >
      <span className={cn("font-medium text-slate-700", uiSize === "sm" && "text-xs")}>{label}</span>
      {children}
